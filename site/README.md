...
**Report a problem** opens a native dialog with an editable Markdown draft. It captures the source, result, language, all rule settings, package version and actual text-column width at the time it opens. Users describe the expected behavior in the draft, copy it, then open GitHub to paste and submit a public issue. The GitHub link has no report data in its URL. There is no automatic submission or reporting backend. Draft edits survive closing and reopening while the playground context stays the same; changing that context creates a fresh report. Invisible characters are escaped in a JSON reproduction block so they survive copying and remain identifiable.

`web-copy.json` holds six attributed, short web-copy samples for a first editorial review. They are test fixtures, not new playground presets. See [the review notes](../docs/research/2026-09-13-web-copy-review.md) for sources, findings and limits.

The playground’s “Watch it wrap” button runs one 4.2-second width comparison, then returns to the starting width. Stop it with the same button, Escape, or a playground control. With reduced motion enabled, “Compare widths” switches instantly between narrow and wide columns. Nothing plays automatically.
