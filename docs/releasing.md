...
All seven packages are released together. Do not run the publish commands until you control the `@typehug` organization and have permission to publish there. Complete authentication and two-factor prompts directly; never put credentials in the repository.

## Prepare

1. Confirm all CI checks pass on Node.js 22 and 24.
2. Confirm the intended version in the root and seven package manifests, exact internal dependency versions, and lockfile. Update the changelog with the release date, API availability notices, website version links, and release feed. Keep earlier changelog and feed entries.
3. Run `npm ci`, `npm run check`, and `npm run site:check` from a clean checkout.
4. Inspect each package with `npm pack --workspace @typehug/core --dry-run` and repeat for `pl`, `en`, `all`, `remark`, `playwright`, and `cli`. Check declarations, JavaScript, license, and README.
...
npm publish --workspace @typehug/remark --access public
npm publish --workspace @typehug/playwright --access public
npm publish --workspace @typehug/cli --access public
```
